...
function confettiBurst(x, y, count = 18) {
  ...
  const parts = Array.from({ length: count }).map(() => ({
    ...
    vy: Math.random() * -6 - 4,
    g: 0.3, s: 3 + Math.random() * 3, a: 1, life: 60 + Math.random() * 30,
  }));
  let frames = 0;
  (function tick() {
    ctx.clearRect(0, 0, c.width, c.height);
    parts.forEach((p) => {
      p.x += p.vx; p.y += p.vy; p.vy += p.g; p.a -= 0.012; p.life--;
...
pokemonRepository.loadList().then(() => {
  ...
  const loadPromises = pokemonRepository.getAll().map((p) =>
    pokemonRepository.loadDetails(p).catch(() => {})
  );
